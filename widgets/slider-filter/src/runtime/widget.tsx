/** @jsx jsx */
import { React, jsx, DataSourceComponent, DataSourceManager, type AllWidgetProps, type ImmutableObject, type FeatureLayerDataSource, type SqlQueryParams } from 'jimu-core'
...
// define the widget as a functional component
const Widget = (props: WidgetProps) => {
  ...
  */
  ...
  const dsManager = useMemo<DataSourceManager>(() => DataSourceManager.getInstance(), [])
  ...
  const [vals, setVals] = useState(null as ImmutableObject<{ [key: string]: number }>)

  // useEffect hook to determine whether the data source is loaded
  useEffect(() => {
    // check whether data source has loaded
    if (useDataSources && useDataSources.length === 1 && useDataSources[0].fields && useDataSources[0].fields.length > 0) {
      // modify the state
      setDsConfigured(true)
    }
    // reset the state on disomunt
    return (): void => { setDsConfigured(false) }
  },
  ...
  useEffect(() => {
    ...
    return (): void => { setVals(null as ImmutableObject<{ [key: string]: number }>) }
  },
  ...
  const setValAndExecuteQuery = (fieldName: string, val: number): void => {
    ...
    const newVals = config.fields.reduce((acc, field) => {
      return { ...acc, [field.name]: field.name === fieldName ? val : vals[field.name] }
    }, {}) as ImmutableObject<{ [key: string]: number }>
    ...
    const sqlExpr = config.fields.reduce((expr: string, field: FilterField, index: number) => {
      if (index === 0) {
        return expr
      }
      return expr.concat(' ', config.logicOp, ' ', field.name, ' >= ', newVals[field.name].toString())
    }, config.fields[0].name.concat(' >= ', newVals[config.fields[0].name].toString()))
  ...
  if (dsConfigured) {
    ...
    return <div className="widget-use-feature-layer" style={{ width: '100%', height: '100%', maxHeight: '800px', overflow: 'auto' }}>
      <div style={{ textAlign: 'center', paddingLeft: '5px', paddingRight: '5px' }}>
        ...
        <div>
          {nls('Set the value for each filter field using the slider.')}
        </div>
      </div>
      <div style={{ paddingTop: '10px' }}>
        <DataSourceComponent
          ...
          config.fields.map(field => <div>
            ...
            <div style={{ textAlign: 'right', paddingRight: '5px', display: 'flex', flexDirection: 'row-reverse' }}>
              <span>
                <input
    ...
    return <h3>
      {nls('Please configure the data source and select fields.')}
    </h3>
  }
}
