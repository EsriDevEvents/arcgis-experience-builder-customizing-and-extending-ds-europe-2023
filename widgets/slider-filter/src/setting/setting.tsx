import { React, Immutable, JimuFieldType, DataSourceManager, ClauseLogic, type SqlQueryParams, type IMFieldSchema, type UseDataSource, type DataRecord, type FeatureLayerDataSource } from 'jimu-core'
...
const Setting = (props: SettingProps) => {
  ...
  */
  ...
  // instantiate National Language Support
  ...
  useEffect(() => {
    ...
    if (useDataSources) {
      ...
      const fetchRecords = async () => {
        ...
        const dataSource = dsManager.getDataSource(useDataSources[0].dataSourceId) as FeatureLayerDataSource
      ...
      fetchRecords().then((res: DataRecord[]): void => { setRecords(res) })
    }
    // reset the state on dismount
    return (): void => { setRecords([] as DataRecord[]) }
  },
  // sole dependency on the data source loading
  [useDataSources])

  // helper function to update properties of the widget config
  const updateConfig = (prop: string, value: ClauseLogic | FilterField[]): void => {
    onSettingChange({
      id: id,
  ...
  const onFieldChange = (allSelectedFields: IMFieldSchema[]) => {
    ...
    onSettingChange({
      id: id,
      useDataSources: [{ ...useDataSources[0], fields: allSelectedFields.map(f => f.jimuName) }]
    })
    // map the fields to new FilterField objects in the widget config
    const fields = allSelectedFields.map((field: IMFieldSchema): FilterField => {
      // read the records form the widget state to find all field values in the data
      const vals = records.map(record => record.getFieldValue(field.jimuName)) as number[]
      // get the minimum field value and round down to the nearest integer multiple of a power of ten
      const minimum = Math.min(...vals)
      const minZeroes = minimum === 0 ? 0 : Math.floor(Math.log10(Math.abs(minimum)))
      const sliderMin = Math.floor(minimum / Math.pow(10, minZeroes)) * Math.pow(10, minZeroes)
      // get the maximum field value and round up to the nearest integer multiple of a power of ten
      const maximum = Math.max(...vals)
      const maxZeroes = maximum === 0 ? 0 : Math.floor(Math.log10(Math.abs(maximum)))
      const sliderMax = Math.ceil(maximum / Math.pow(10, maxZeroes)) * Math.pow(10, maxZeroes)
      return {
        name: field.jimuName,
  ...
  const renderOpSetting = () => {
    return (
      <SettingSection className='map-selector-section'>
        <SettingRow flow='wrap' label={nls('Logical Operator')}>
          <div className='d-flex w-100 select-mode-con'>
            <div className='flex-grow-1 text-truncate'>
            <Button className='w-100' active={config.logicOp === ClauseLogic.And} onClick={() => { onLogicOpChange(ClauseLogic.And) }}>
                {nls(ClauseLogic.And)}
              </Button>
            </div>
            <div className='flex-grow-1 ml-2 text-truncate'>
              <Button className='w-100' active={config.logicOp === ClauseLogic.Or} onClick={() => { onLogicOpChange(ClauseLogic.Or) }}>
                {nls(ClauseLogic.Or)}
              </Button>
            </div>
          </div>
